...
function enterLobby(gameName) {
  ...
  if (isOwner) {
    ...
    setTimeout(initSetupMap, 50); // allow DOM to render before initializing map
  } else {
...
function initSetupMap() {
  ...
  setupMap = L.map('setup-map').setView([44.2619, -88.4154], 13); // default: Appleton, WI
  L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
    attribution: '© OpenStreetMap contributors',
    maxZoom: 19,
  }).addTo(setupMap);

  // Try to recenter on the user's actual location
...
function updateGame(state) {
  ...
  // Update other players' markers (only visible if outside zone)
  ...
  // Update my "inside/outside zone" badge
  if (lastLocation) {
    ...
    status.textContent = inside ? 'Inside Zone' : 'OUTSIDE — VISIBLE!';
